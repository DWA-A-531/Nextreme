...
Trend_vs_Sprung = function(Zeit,Serienwerte,Sensor=NULL,ifTS=T,ifAnova=F,skaliereZeit = T, PrintLine=NULL){
  require(evd)
  require(lmomco)
  Fgev=function(...){ res=list(mo="",Aic=as.numeric(NA),Bic=as.numeric(NA));  mo=try(evd::fgev(...,std.err=F),sil=T); if(!is.character(mo)){mo$Aic=AIC(mo);mo$Bic=AIC(mo,k=length(mo$est))};  mo }
  Anova=function(...){res=try(anova(...)[2,5]);if(is.character(res))res=as.numeric(NA);res}
  ...
  DSP=DTRSP=""
  if(!is.null(Sensor)){
    DSP   = try(data.frame(model.matrix(~factor(Sensor),data.frame(Sensor)))[,-1],sil=T) # ohne intercept
