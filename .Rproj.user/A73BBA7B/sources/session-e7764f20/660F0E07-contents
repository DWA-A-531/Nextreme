#' Calculating the GEV parameters with a fixed shape (kappa parameter)
#' @param lmom the L-moments calculated previously with the lmomco package
#' @param kappa the fixed value of the kappa parameter (shape parameter) if kappa=NULL that it is calculated according to the Lmoments
#' @details
#' R-Function to calculate calculate the GEV parameters when the shape parameter (kappa) is set as a certain value.
#' @return  the gev parameters
#' @examples
...
pargev2 = function (lmom, checklmom = TRUE, kappa=NULL, ...) { # aus lmomco, erweitert um kappa = NULL, wenn kappa != NULL, dann wird mit fixiertem kappa gesch?tzt
  require(lmomco)
  para <- rep(NA, 3)
