...
#' @param Serie jaehrliche Maximum Serie (Regenintensitaet in mm/h) als Tabelle, wo die Anzahl der Zeilen die Jahre mit verfuegbaren Daten und die Anzahl der Spalten die ausgewaehlten Dauern bezeichnen.
...
#' @examples
#' # Berechnung der Starkregenparameter fuer die Station Goerlitz im Zeitraum 1991-2020, ohne Intervall-oder Sprungkorrektur
...
#' print(extremParameter)
#' # Fall 3: ueber Dauern von mehr als einer Stunde mit der GEV-Verteilung und einem nicht festgelegten Formparameter.
...
Parameter_Schaetzung = function(Serie, Dauern=c(5, 10, 15,30,60,120,360,720,1440, 2880, 4320, 10080),
  ...
  Theta.Werte = optimize(kw_koupar1, lower=0, upper=1,  Inten.Daten = Inten.Daten, Dauern=Dauern_inStunden, Partition = Partition, nD=nD, m=m, maximum=FALSE)
  Eta.Werte = optimize(kw_koupar2, lower=0, upper=1, Dauern = Dauern_inStunden, Theta=Theta.Werte$minimum, Inten.Daten = Inten.Daten,Partition=Partition,nD=nD,m=m, maximum=FALSE)
