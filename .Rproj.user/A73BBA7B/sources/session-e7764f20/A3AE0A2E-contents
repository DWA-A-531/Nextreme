...
#' @details
#' Wenn es einen Sprung gibt - Typ Instaionaritaet auf der jaehrlichen Serie von kurzen Dauern, wird der Sprung eliminiert und eine korrigierte jaehrliche Serientabelle zurueckgegeben.
#' @return
...
Sprung_Korrektur  <- function(Serie, Sensorwechsel){
  ...
  else if(length(which(as.numeric(colnames(Serie))<=30))<1) stop("Das Serie Input sollte mindestens eine Spalte mit einer Dauer kleiner oder gleich 30 Minuten enthalten! Die Funktion „sprungKorrektur“ wird nur auf die Spalten angewendet, deren Namen kuerzer oder gleich 30 Minuten sind.")
  ...
  suppressWarnings({
  if(Wechseljahr>alleJahren[1] & Wechseljahr< tail(alleJahren,1)){
    ...
    Serie[,which(Dauern<=30)] = apply(Serie[,which(Dauern<=30)], 2, function(x){
      ...
      x_out = x
      if(SIG.TEST$AicRes == "Sprung") x_out[which(is.na(x)==F)] = Sprung_Elimination(x[which(is.na(x)==F)],sensor_vec[which(is.na(x)==F)], ZielSensor = sensor_vec[length(sensor_vec)])$SerieNeu
